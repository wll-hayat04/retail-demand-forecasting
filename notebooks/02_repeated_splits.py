...
repeats.to_csv(config.RESULTS / "repeated_splits_raw.csv", index=False)

summary = val.summarise_repeats(repeats)
summary.to_csv(config.RESULTS / "repeated_splits_summary.csv", index=False)
print(summary.to_string(index=False))

print()
for cat in config.SELECTED_CATEGORIES:
    sub = repeats[repeats["Category"] == cat]
    d = val.is_difference_meaningful(sub, "Random Forest", "7-Day Rolling Sum Baseline")
    print(cat, "| RF vs Baseline:", d)
